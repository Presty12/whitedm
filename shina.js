...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Öneri/Şikayet İçin Dm | Sunucu üyelerinden gelen direkt mesajlara izin vermeniz gerekmektedir!" } });
...
client.on("message", async message => {
  ...
  if (message.channel.type === "dm" && (koruma == null || koruma == undefined || koruma == false)) {
    ...
    let embed = new Discord.MessageEmbed().setColor("RANDOM").addField("Yeni Öneri/Şikayet Geldi!", `${message.content ? message.content.slice(0, 900) : ""}\n\n\`Öneriyi Yapan:\` ${message.author}`);
